...
from conciliador_engine import (
    ...
    agrupar_datafono_por_abono, cruzar_auxiliar_datafono_por_abono,
...
class ConciliadorApp(tk.Tk):
    def __init__(self):
        ...
        self._modo_fecha = tk.StringVar(value="FECHA_VALE")
    ...
    def _run_uno(self, year, mes):
        try:
            ...
            modo_fecha = self._modo_fecha.get()
            self._set_prog(55, f"Cruzando ({modo_fecha})…")
            if modo_fecha == "FECHA_ABONO":
                por_dia, por_bolruta = agrupar_datafono_por_abono(df_dat)
                resultados = cruzar_auxiliar_datafono_por_abono(df_aux, por_dia, por_bolruta, nombre_arch, year, mes)
            else:
    ...
    def _run_todos(self, year, mes):
        try:
            ...
            modo_fecha = self._modo_fecha.get()
            self._log(f"  Modo de cruce: {modo_fecha}")
            if modo_fecha == "FECHA_ABONO":
                por_dia, por_bolruta = agrupar_datafono_por_abono(df_dat)
            else:
                por_dia, por_bolruta = agrupar_datafono_por_dia_vale(df_dat)
            ...
            for i, (nombre_arch, info) in enumerate(mapa.items()):
                ...
                if modo_fecha == "FECHA_ABONO":
                    resultados = cruzar_auxiliar_datafono_por_abono(

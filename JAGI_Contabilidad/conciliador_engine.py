"""
Motor de Conciliación de Datafonos — v1.6
Empresa: GIRALDO GIRALDO JAIME WILSON  |  Normativa: NIIF PYMES / DIAN Colombia

NOVEDADES v1.6:
  • Criterio de cruce de fechas configurable por la auxiliar contable:
      - FECHA_VALE:  Nota.Dia_Operacion == datafono.Fecha_Vale.day   (estándar NIIF)
      - FECHA_ABONO: Nota.Dia_Operacion == datafono.Fecha_Abono.day  (por acreditación bancaria)
  • Ambos modos son operativamente válidos según el criterio de registro del período
  • Log de auditoría refleja el criterio usado sin distinción de jerarquía entre modos
  • Formato de Nota vigente (Marzo 2026+): dd/mm/aaaa DATAFONO — sede en campo Tercero
  • Detección automática de formato por fila: compatible con todos los formatos históricos
...
COLOR = {
    ...
    "cuadra_otro_mes":      "B8CCE4", "cuadra_otro_mes_f":  "1F3864",
    "proximo_mes":          "D9D2E9", "proximo_mes_f":      "351C75",
...
def _extraer_mes_de_nota(nota: str) -> int | None:
    """
    Extrae el mes desde la Nota, SOLO si hay una fecha completa (dd/mm/aaaa o dd-mm-aaaa)
    o fecha parcial (dd/mm o dd-mm) embebida. No intenta adivinar el mes desde formatos
    ambiguos (ej: "2- DATAFONO ENVIGADO") para no generar falsos positivos.
    Retorna None si la Nota no contiene una fecha con mes explícito.
    """
    n = str(nota).strip()
    # Fecha completa dd/mm/aaaa  o  dd-mm-aaaa
    m = re.search(r'\b(\d{1,2})[/\-](\d{1,2})[/\-](\d{2,4})\b', n)
    if m:
        mes_val = int(m.group(2))
        return mes_val if 1 <= mes_val <= 12 else None
    # Fecha parcial dd/mm  o  dd-mm
    m = re.search(r'\b(\d{1,2})[/\-](\d{1,2})\b', n)
    if m:
        mes_val = int(m.group(2))
        return mes_val if 1 <= mes_val <= 12 else None
...
def cruzar_auxiliar_datafono(df_aux, por_dia, por_bolruta,
    ...
    for _, ra in df_aux.iterrows():
        ...
        # ── Detectar si la Nota indica un mes distinto al período ──────────────
        nota_str = str(ra.get('Nota', '')).strip()
        mes_nota = _extraer_mes_de_nota(nota_str)
        es_otro_mes = (mes_nota is not None and mes_nota != mes)

        # Mes de búsqueda: usar el mes indicado en la nota si es distinto
        mes_busqueda  = mes_nota if es_otro_mes else mes
        year_busqueda = (year + 1) if (es_otro_mes and mes_nota < mes) else year

        match_dia = por_dia[
            (por_dia['_Nombre_Archivo'].str.upper() == nombre_archivo.upper()) &
            (por_dia['_Dia_Vale']  == dia)  &
            (por_dia['_Year_Vale'] == year_busqueda) &
            (por_dia['_Mes_Vale']  == mes_busqueda)
        ]
        ...
        cands = por_bolruta[
            ...
            (por_bolruta['_Year_Vale'] == year_busqueda) &
            (por_bolruta['_Mes_Vale']  == mes_busqueda)
        ].copy()

        if es_otro_mes:
            if len(match_dia) > 0 and (abs(dif) < 1 or abs(dif) <= v_aux * 0.05):
                estado = 'CUADRA_OTRO_MES'
            else:
                # Fallback: buscar por Fecha_Abono en el mes/dia que indica la nota
                # Caso: Fecha_Vale=último día del mes anterior, Fecha_Abono=día X del mes siguiente
                match_abono = por_bolruta[
                    (por_bolruta['_Nombre_Archivo'].str.upper() == nombre_archivo.upper()) &
                    (por_bolruta['Fecha de Abono'].dt.day   == dia) &
                    (por_bolruta['Fecha de Abono'].dt.month == mes_busqueda) &
                    (por_bolruta['Fecha de Abono'].dt.year  == year_busqueda)
                ] if 'Fecha de Abono' in por_bolruta.columns else pd.DataFrame()

                if len(match_abono) > 0:
                    s_gr_ab = float(match_abono['Valor Neto'].sum())
                    dif_ab  = v_aux - s_gr_ab
                    if abs(dif_ab) < 1 or abs(dif_ab) <= v_aux * 0.05:
                        s_gr  = s_gr_ab
                        dif   = dif_ab
                        cands = match_abono.copy()
                        estado = 'CUADRA_OTRO_MES'
                    else:
                        estado = 'PROXIMO_MES_SIN_MATCH'
                else:
                    estado = 'PROXIMO_MES_SIN_MATCH'
        else:
...
def cruzar_auxiliar_datafono_por_abono(df_aux, por_abono, por_bolruta,
    ...
    for _, ra in df_aux.iterrows():
        ...
        match_abono = por_abono[
            ...
            (por_abono['_Year_Abono'] == year_busqueda) &
            (por_abono['_Mes_Abono']  == mes_busqueda)
        ]
        ...
        cands = por_bolruta[
            ...
            (por_bolruta['_Year_Abono'] == year_busqueda) &
            (por_bolruta['_Mes_Abono']  == mes_busqueda)
        ].copy()

        if es_otro_mes:
            if len(match_abono) > 0 and (abs(dif) < 1 or abs(dif) <= v_aux * 0.05):
                estado = 'CUADRA_OTRO_MES'
            else:
                estado = 'PROXIMO_MES_SIN_MATCH'
        else:
...
def _hoja_detalle(wb, resultados, sede, periodo, mes, ws_name="DETALLE_CONCILIACION"):
    ...
    for res in resultados:
        ...
        elif estado == 'CUADRA_OTRO_MES':
            bg,fc,obs = COLOR["cuadra_otro_mes"],  COLOR["cuadra_otro_mes_f"],  f"✔ Cuadra — transacción de mes {mes_real} conciliada en este período"
        elif estado == 'PROXIMO_MES_SIN_MATCH':
            bg,fc,obs = COLOR["proximo_mes"],      COLOR["proximo_mes_f"],      f"🔜 Nota indica mes {mes_real} — sin valor correspondiente en datafono"
        else:
...
def _hoja_resumen_individual(wb, resultados, sede, periodo, info_match=None):
    ...
    _ESTADOS_OK      = ('CUADRA', 'DIF_MENOR', 'CUADRA_OTRO_MES')
    _ESTADOS_REVISAR = ('DIFERENCIA', 'SIN_MATCH', 'SIN_DIA', 'PROXIMO_MES_SIN_MATCH')

    cuadra         = sum(1 for r in resultados if r['estado'] == 'CUADRA')
    dif_menor      = sum(1 for r in resultados if r['estado'] == 'DIF_MENOR')
    cuadra_otro_mes= sum(1 for r in resultados if r['estado'] == 'CUADRA_OTRO_MES')
    ...
    prox_sin_match = sum(1 for r in resultados if r['estado'] == 'PROXIMO_MES_SIN_MATCH')
    ...
    pct_ok = ((cuadra + dif_menor + cuadra_otro_mes) / total_reg * 100) if total_reg > 0 else 0
    stats = [
        ...
        ("✔  Cuadra — transacción de mes siguiente conciliada",     cuadra_otro_mes,  None,      COLOR["cuadra_otro_mes"]),
        ...
        ("🔜  Nota indica otro mes — sin valor en datafono",        prox_sin_match,   None,      COLOR["proximo_mes"]),
        (f"📊  % registros cuadrados (incl. otro mes)",            f"{pct_ok:.1f}%", None,      None),
    ]
...
def _hoja_auditoria(wb, sede, periodo, info_match=None, es_consolidado=False,
    ...
    if modo_fecha == 'FECHA_ABONO':
        logica_cruce  = "Nota.Dia_Operacion == Fecha_Abono.day (mismo mes y año)"
        agrupacion_df = "Por día Fecha de Abono → SUM(Valor Neto)"
        nota_cruce    = ("Criterio seleccionado: Fecha de Abono. "
                         "La Nota del auxiliar contiene la fecha de acreditación bancaria. "
                         "Consistente con el modo de registro del período conciliado.")
    else:
        logica_cruce  = "Nota.Dia_Operacion == Fecha_Vale.day (mismo mes y año)"
        agrupacion_df = "Por día Fecha Vale → SUM(Valor Neto)"
        nota_cruce    = ("Criterio seleccionado: Fecha Vale. "
                         "La Nota del auxiliar contiene la fecha de la venta. "
                         "Principio de devengo NIIF PYMES.")
    ...
    registros = [
        ...
        ("Marca comercial",               info_empresa.get("marca_comercial", "") if info_empresa else ""),
        ("Cuenta bancaria conciliada",    info_empresa.get("cuenta", "") if info_empresa else ""),
...
def _hoja_pendientes(wb, resultados, por_dia, por_bolruta,
    ...
    pendientes_aux = [r for r in resultados
                      if r['estado'] in ('SIN_MATCH','DIFERENCIA','SIN_DIA','PROXIMO_MES_SIN_MATCH')]
    ...
    # Construir índice (mes, año, dia) de todos los registros del auxiliar
    # para excluir correctamente incluso los de otro mes
    dias_en_aux = set()  # set de (year, mes, dia) ya cubiertos por el auxiliar
    for res in resultados:
        d = res['row_aux']['Dia_Operacion']
        if pd.notna(d):
            mes_r  = res.get('mes_real', mes)
            year_r = (year + 1) if (mes_r < mes) else year
            dias_en_aux.add((year_r, mes_r, int(d)))

    # Detectar qué columnas de fecha tiene por_dia (depende del modo FECHA_VALE o FECHA_ABONO)
    if '_Year_Vale' in por_dia.columns:
        col_year, col_mes, col_dia = '_Year_Vale', '_Mes_Vale', '_Dia_Vale'
    else:
        col_year, col_mes, col_dia = '_Year_Abono', '_Mes_Abono', '_Dia_Abono'

    mes_siguiente  = mes + 1 if mes < 12 else 1
    year_siguiente = year + 1 if mes == 12 else year

    mask_periodo  = (
        (por_dia['_Nombre_Archivo'].str.upper() == nombre_archivo.upper()) &
        (por_dia[col_year] == year) &
        (por_dia[col_mes]  == mes)
    )
    mask_siguiente = (
        (por_dia['_Nombre_Archivo'].str.upper() == nombre_archivo.upper()) &
        (por_dia[col_year] == year_siguiente) &
        (por_dia[col_mes]  == mes_siguiente)
    )
    df_candidatos = por_dia[mask_periodo | mask_siguiente].copy()

    df_sin_aux = df_candidatos[
        ~df_candidatos.apply(
            lambda r: (int(r[col_year]), int(r[col_mes]), int(r[col_dia])) in dias_en_aux,
            axis=1
        )
    ].copy()
    ...
    h2 = ws.cell(row=row_n, column=1,
                 value=f"B)  EN DATAFONO SIN REGISTRO EN AUXILIAR ({len(df_sin_aux)} días — incluye abonos de mes siguiente si aplica)")
    ...
    for _, gr in df_sin_aux.iterrows():
        ...
        fecha_val = f"{int(gr[col_dia])}/{int(gr[col_mes])}/{int(gr[col_year])}"
        obs_b = ("Abono recibido en banco sin registro contable en el auxiliar"
                 if gr[col_mes] == mes
                 else f"Abono de mes {int(gr[col_mes])} en datafono sin contrapartida en auxiliar")
        vals = [fecha_val, v_neto,
...
def generar_excel_resultado(resultados: list, sede_nombre: str,
    ...
    _hoja_detalle(wb, resultados, sede_nombre, periodo, mes)
    ...
    _hoja_auditoria(wb, sede_nombre, periodo, info_match, modo_fecha=modo_fecha, info_empresa= None)
